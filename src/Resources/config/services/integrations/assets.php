...
return static function (ContainerConfigurator $container) {
    ...
    $services->set(AssetsInstallerInterface::class, AssetsInstaller::class)
        ->args([
            service('filesystem'),
            service('kernel'),
    ...
    $services->set(OutputAwareAssetsInstaller::class)
        ->decorate(AssetsInstallerInterface::class, null, 256)
        ->args([service('Sylius\Bundle\ThemeBundle\Asset\Installer\OutputAwareAssetsInstaller.inner')]);

    $services->set(PathResolverInterface::class, PathResolver::class)
        ->args([
            service(AssetsProviderInterface::class),
            service(FilesystemInterface::class),
        ]);

    $services->set(AssetsProviderInterface::class, AssetsProvider::class)
        ->args([
            service('kernel'),
